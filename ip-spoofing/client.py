...
def main():
	...
	hw = bytes.fromhex(my_mac.replace(':', ''))
	...
	# pkt 6: a non-DHCP packet with legitimate IP but forged source MAC
	# DROPPED by stricter (src IP + src MAC + ingress port) binding
	pkt6 = Ether(src='00:00:00:00:aa:aa', dst='ff:ff:ff:ff:ff:ff') / IP(src='10.0.1.1', dst='255.255.255.255') / TCP(dport=1234, sport=random.randint(49152,65535))
	pkt6.show2()
